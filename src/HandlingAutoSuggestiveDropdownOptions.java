...
//		}
		...
		try {
			int flag = 0;
			driver.findElement(By.xpath("//input[@name='fromPlaceName']")).sendKeys("BEN");
			for(int i = 0; i<20; i++) {
				driver.findElement(By.xpath("//input[@name='fromPlaceName']")).sendKeys(Keys.DOWN);
				Thread.sleep(1000L);
				JavascriptExecutor js = (JavascriptExecutor)driver;
				String value = (String)js.executeScript("return document.getElementById(\"fromPlaceName\").value;");
				System.out.println(value);
				if(value.equalsIgnoreCase("BENGALURU ITERNATION AIPORT")) {
					System.out.println("Value " + value + " found successfully");
					flag  = 1;
					break;
				}
			}
